...
do_control_doors=1
import os
print './prep_video_gige_wcam.py'
execfile("./prep_video_gige_wcam.py")
...
def ram_2AFC(closeall=[],openall=[],moveall=[],close_slowly=[],ntrials=1,ppal_stim=[],ppal_cam=[]):
    ...
    #stop_if_error=True # TODO get this as input
    #timeout_arm_sec= 60 # when to time out if spending too much time in an arm
    if is_wcam is True:
        dist_center_safety=4.7e4
    else:
    ...
    thefilename,trials,do_stim,lendelay,ndoors_to_open,stop_if_error,timeout_arm_sec=get_filename_delay()
    ...
    if do_control_doors:
        if behavioral_task!=3 and behavioral_task!=4:
            ...
            for door in doors_to_open_2afc: moveall(door,1)
    arms_visited=[];    time_last=time.time()
    while state=='Training': # the trial has to be less than 5 mins:
        ...
        curnarm,curcoord,frame_and_info,sample_gige,sample,ram_should_continue,Curnarm=read_and_show(Frame,1,diderrors,str(int(elapsed_from_last)),Curnarm)
        # Timeout?
        ...
        elapsed_from_last= time.time()- time_last; #time_last=time.time();  #print "Time from last visit: ", elapsed_from_last
        if elapsed_from_last>timeout_arm_sec and curnarm !=0:
            print "Spent too long in arm: " , elapsed_from_last, time.time()-trial_starts ;   state="wait_for_safe2closelast"
            if not (curnarm==999 or curnarm==0):                    lastarm=curnarm
            else:                    lastarm=prevnarm
            print 'last arm was ' , lastarm;                lastdoor=armcoords[lastarm-1];
        ...
        if  curnarm!=prevnarm and (curnarm!=999) and (curnarm!=0)  and (curnarm!=111) and (curnarm in doors_to_open):
            ...
            if curnarm in arms_to_visit:
                ...
                if stop_if_error is True:
                    print "Requested reset when error; exiting" ; state="wait_for_safe2closelast"
                    if not (curnarm==999 or curnarm==0):                    lastarm=curnarm
                    else:                    lastarm=prevnarm
                    print 'last arm was ' , lastarm;                lastdoor=armcoords[lastarm-1];
    ...
    while state=='delay'  and elapsed<60*(timeout) and not doors_open==range(1,total_narms+1): # the trial has to be less than timeout:
        ...
        if cam_on:
            curnarm,curcoord,frame_and_info,sample_gige,sample,ram_should_continue,Curnarm=read_and_show(Frame,1,diderrors,str(int(time.time() - while_starts))+ ' of '+str(int(lendelay)),Curnarm)
            if ram_should_continue==False: exit_nicely(ppal_cam); break
